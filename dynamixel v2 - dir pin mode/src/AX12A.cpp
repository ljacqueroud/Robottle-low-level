...
{
  pinMode(RX_PIN, OUTPUT);            // Set RX to not receive anything
  ...
  pinMode(RX_PIN, INPUT);             // Allow RX to receive
...
int AX12A::writeRegister2(unsigned char ID, unsigned char reg, int regValue)
{
	...
	Checksum = (~(ID + AX_WRITE_REG_LENGTH2 + AX_WRITE_DATA + reg + regValue_L + regValue_H)) & 0xFF;

	packet[0] = AX_START;
	packet[1] = AX_START;
	packet[2] = ID;
	packet[3] = AX_WRITE_REG_LENGTH2;
	packet[4] = AX_WRITE_DATA;
	packet[5] = reg;
	packet[6] = regValue_L;
	packet[7] = regValue_H;
